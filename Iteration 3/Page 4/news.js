$(document).ready(function () {
    //var url = 'https://newsapi.org/v2/everything?q=hpv&apiKey=8c2e107ceae84471984fa6953948cee5';
        ...
        if (res.articles.length > 0) {
            ...
            if (res.articles.length > 12) {
                for (var i = 0; i < 12; i++) {
                    ...
                    s += `<a href="#foo" onclick="window.open('${res.articles[i].url}', '_blank', 'width=1000, height=800')"><img src=${res.articles[i].urlToImage} alt="logo" style="width: 100%;height:95%"></a>`;
                    s += "</div>";
                    s += `<h6>${res.articles[i].title}</h6>`;
                    s += "</div>";
                }
            }
            else {
